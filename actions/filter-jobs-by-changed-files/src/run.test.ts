import { matchesDependency, resolveDependencyPaths, jobMatchesChangedFiles, PathInfo } from './run';
...
describe('matchesDependency', () => {
  describe('directory dependencies', () => {
    it('matches files within the directory', () => {
      expect(matchesDependency('apps/foo/src/main.go', 'apps/foo/', true)).toBe(true);
      expect(matchesDependency('apps/foo/pkg/utils/helper.go', 'apps/foo/', true)).toBe(true);
    });

    it('does not match files outside the directory', () => {
      expect(matchesDependency('apps/bar/src/main.go', 'apps/foo/', true)).toBe(false);
      expect(matchesDependency('apps/foobar/main.go', 'apps/foo/', true)).toBe(false);
    });

    it('handles directories without trailing slash', () => {
      expect(matchesDependency('apps/foo/main.go', 'apps/foo', true)).toBe(true);
      expect(matchesDependency('apps/foo/src/main.go', 'apps/foo', true)).toBe(true);
    });
  });
  ...
  describe('file dependencies', () => {
    it('matches exact file path', () => {
      expect(matchesDependency('apps/foo/go.mod', 'apps/foo/go.mod', false)).toBe(true);
      expect(matchesDependency('apps/foo/package.json', 'apps/foo/package.json', false)).toBe(true);
    });

    it('does not match similar but different files', () => {
      expect(matchesDependency('apps/foo/go.mod.backup', 'apps/foo/go.mod', false)).toBe(false);
      expect(matchesDependency('apps/foo/go.module', 'apps/foo/go.mod', false)).toBe(false);
    });

    it('does not match files within a directory with the same name', () => {
      // Files should only match exactly
      expect(matchesDependency('apps/foo/go.mod/nested.txt', 'apps/foo/go.mod', false)).toBe(false);
    });
  });
});
...
describe('resolveDependencyPaths', () => {
  const mockGetPathInfo = (path: string): PathInfo => {
    ...
    if (path.endsWith('.mod') || path.endsWith('.json') || path.endsWith('.txt')) {
  ...
  it('resolves dependency paths correctly', () => {
    ...
    const result = resolveDependencyPaths(dependencies, 'apps', mockGetPathInfo);
...
describe('jobMatchesChangedFiles', () => {
  ...
  it('matches when files are within app path', () => {
    ...
    expect(jobMatchesChangedFiles(job, changedFiles, 'apps', dependencyPathInfos)).toBe(true);
  });
  ...
  it('does not match when no relevant files change', () => {
    ...
    expect(jobMatchesChangedFiles(job, changedFiles, 'apps', dependencyPathInfos)).toBe(false);
  });

  it('matches exact file dependencies', () => {
    const job = createTestJob('apps/web-app/cmd/api-server', ['web-app/go.mod', 'web-app/go.sum']);
  ...
  it('does not match partial file paths', () => {
    const job = createTestJob('apps/web-app/cmd/api-server', ['web-app/go.mod']);
